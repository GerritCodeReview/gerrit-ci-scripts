...
def call(Map parm = [:]) {
    ...
    def pluginScmBaseUrl = "https://gerrit.googlesource.com/a/plugins"
    ...
    pipeline {
        ...
        stages {
			...
			extraPlugins.each { plugin -> sh "[ -d gerrit/plugins/${plugin} ] || (git clone -b ${BRANCH_NAME} ${pluginScmBaseUrl}/${plugin} && cd gerrit/plugins && ln -sf ../../${plugin} .)" }
                    }
                }
            }
            stage('Formatting') {
