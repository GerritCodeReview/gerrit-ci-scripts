...
pipeline {
       ...
       environment {
            ...
            REPLICA_SUBDOMAIN = String.format("%s-%s-replica.%s", "jenkins", epochTime, "${params.BASE_SUBDOMAIN}") //$(AWS_PREFIX)-replica.gerrit-demo

            GERRIT_HTTP_URL = String.format("%s://%s.%s", "${params.GERRIT_HTTP_SCHEMA}", HTTP_PRIMARIES_GERRIT_SUBDOMAIN, "${params.HOSTED_ZONE_NAME}")
            GERRIT_SSH_URL = String.format("ssh://%s@%s.%s", "${params.GERRIT_SSH_USERNAME}", SSH_PRIMARIES_GERRIT_SUBDOMAIN, "${params.HOSTED_ZONE_NAME}")
         }
                ...
                withCredentials([usernamePassword(usernameVariable: "AWS_ACCESS_KEY_ID", 
                    ...
                    credentialsId: "aws-credentials-id")]) {
                        dir ('aws-gerrit/dual-primary') {
                            ...
                            sh "make AWS_REGION=${params.AWS_REGION} AWS_PREFIX=${params.AWS_PREFIX} delete-git-replica-persistent-stack"
                            sh "make AWS_REGION=${params.AWS_REGION} AWS_PREFIX=${params.AWS_PREFIX} delete-network-persistent-stack"
                        }
                }
            }
        }
