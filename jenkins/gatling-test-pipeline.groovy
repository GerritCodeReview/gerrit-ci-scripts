            ...
            stage('Extract Gatling test user credentials from Gerrit') {
                steps {
                    retry(50) {
                        ...
                        sh """
                            set +x
                            curl -L -c cookies -i ${env.GERRIT_HTTP_URL}/login/%2Fq%2Fstatus%3Aopen%2B-is%3Awip' --data-raw 'username=${params.GERRIT_LOGIN_USERNAME}&password=${params.GERRIT_LOGIN_PASSWORD}'
                            set -x
                        """
                    }
                    script {
