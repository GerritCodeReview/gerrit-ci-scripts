...
pipeline {
    ...
    stages {
        stage('Checkout JGit') {
            steps {
               withCredentials([usernamePassword(usernameVariable: "GS_GIT_USER", passwordVariable: "GS_GIT_PASS", credentialsId: "eclipse.gerrithub.io")]) {
                   sh 'echo "machine eclipse.gerrithub.io login $GS_GIT_USER password $GS_GIT_PASS">> ~/.netrc'
                   sh 'chmod 600 ~/.netrc'
                   sh 'git clone -b servlet-4 "https://eclipse.gerrithub.io/a/eclipse-jgit/jgit"'
               }
            }
        }
        stage('Build JGit') {
