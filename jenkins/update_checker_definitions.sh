...
-d '{
    ...
    "repository": "gerrit",
    "query": "(not dir:polygerrit-ui) AND (branch:stable-3.11 OR branch:stable-3.12 OR branch:stable-3.13 OR branch:stable-3.14 OR branch:master)",
    "blocking": []
  }'
...
curl -X POST -n -v https://gerrit-review.googlesource.com/a/plugins/checks/checkers/gerritforge%3Arbe-a6a0e4682515f3521897c5f950d1394f4619d928  \
  -H 'Content-Type: application/json; charset=UTF-8' -d '
  {
    ...
    "repository": "gerrit",
    "query": "(not dir:polygerrit-ui)  AND -age:1w AND (branch:stable-3.11 OR branch:stable-3.12 OR branch:stable-3.13 OR branch:stable-3.14 OR branch:master)",
    "blocking": []
  }'
...
curl -X POST -n -v https://gerrit-review.googlesource.com/a/plugins/checks/checkers/gerritforge%3Acodestyle-a6a0e4682515f3521897c5f950d1394f4619d928  \
  -H 'Content-Type: application/json; charset=UTF-8' -d '
  {
    ...
    "repository": "gerrit",
    "query": "branch:stable-3.11 OR branch:stable-3.12 OR branch:stable-3.13 OR branch:stable-3.14 OR branch:master",
    "blocking": []
  }'
