...
def listChangedProjects() {
    ...
    def lastBuildStartTimeMillis = lastBuild == null ?
            (System.currentTimeMillis() - TimeUnit.MINUTES.toMillis(pollMinutes + 1)) : lastBuild.getStartTimeInMillis()
    def sinceMillis = lastBuildStartTimeMillis - TimeUnit.MINUTES.toMillis(1)
...
node('master') {
    ...
    listChangedProjects().each { project ->
        stage("${project}") {
            ...
            sh "curl -v -d '${jsonPayload}' $hookUrl"
        }
    }
}
