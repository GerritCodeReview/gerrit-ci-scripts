...
class Gerrit {
    ...
    int applyVerificationLabel(verificationScore, builds) {
    ...
    int applyCodestyleLabel(codeStyleScore, files, build) {
        if(codeStyleScore == 0) {
            return
        }
...
def buildChange() {
    ...
    gerrit.applyVerificationLabel(verificationScore, Builds.buildsList.findAll { key,build -> key != "codestyle" })
...
def retryFlakyBuilds(flaky){
    ...
    toolsAndModes.each {
        ...
        retryBuilds += formatBuildStep(buildName,mode,Config.retryTimes)
    }
