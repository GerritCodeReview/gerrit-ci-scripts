...
abstract class AbstractLabel {
  ...
  AbstractLabel(label, script, score) {
    this.label = label
    this.script = script
    this.score = score
  }
...
class VerifyLabel extends AbstractLabel {
  ...
  VerifyLabel(script, score) {
    super("Verified", script, score)
  }

  @NonCPS
  def createMsgBody() {
    def msgList = Builds.buildsVerification.collect { type,build ->
      [ 'type': type, 'res': build.getResult().toString(), 'url': build.getUrl() + "consoleText" ]
    }
...
class CodeStyleLabel extends AbstractLabel {
  ...
  CodeStyleLabel(script, score) {
    super("Code-Style", script, score)
  ...
  def createMsgBody() {
    ...
    def res = Builds.buildCodeStyle.getResult().toString()
    def url = Builds.buildCodeStyle.getUrl() + "consoleText"
...
node('master') {
    ...
    stage('Verification'){
        ...
        parallel Builds.modes.collectEntries {
            ["Gerrit-verification(${it})" : prepareBuildsForMode("Gerrit-verifier-${Builds.tool}", it)]
        }
    }
    ...
    stage('Vote'){
        if(Builds.resultCodeStyle) {
            def resCodeStyle = getLabelValue(1, Builds.resultCodeStyle)
