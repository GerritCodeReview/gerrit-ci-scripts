...
def buildChange() {
  ...
  if (Config.codeStyleBranches.contains(Change.branch)){
    builds += prepareBuildsForMode("Gerrit-codestyle")
  }
  ...
  Builds.modes.collect {
    prepareBuildsForMode(buildName,it)
  }.each { builds += it }
  ...
  gerrit.addVerifiedLabel(resVerify, Builds.buildsList.findAll { key,build -> key != "codestyle" })
...
def retryFlakyBuilds(flaky){
    ...
    toolsAndModes.each {
      ...
      retryBuilds += prepareBuildsForMode(buildName,mode,3)
    }
