...
class Gerrit {
  ...
  def addLabel(changeNum, sha1, label, score, msgBody = "") {
...
def execShellCommand(cwd, command) {
...
def buildChange(change) {
  def gerrit = new Gerrit("https://gerrit-review.googlesource.com/")
  ...
  execShellCommand(cwd, "git fetch origin $ref")
  execShellCommand(cwd, "git checkout FETCH_HEAD")
  execShellCommand(cwd, "git fetch origin $branch")
  execShellCommand(cwd, 'git config user.name "Jenkins Build"')
  execShellCommand(cwd, 'git config user.email "jenkins@gerritforge.com"')
  execShellCommand(cwd, 'git merge --no-commit --no-edit --no-ff FETCH_HEAD')
