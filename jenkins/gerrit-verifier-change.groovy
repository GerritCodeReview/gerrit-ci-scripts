...
class Gerrit {
  ...
  int applyVerificationLabel(verified, builds) {
    ...
    if(addVerifiedExit == 0) {
      this.script.println "----------------------------------------------------------------------------"
      this.script.println "Gerrit Review: Verified=" + verified + " to change " + Change.changeJson._number +
        "/" + Change.sha1
      this.script.println "----------------------------------------------------------------------------"
    }
  ...
  int applyCodestyleLabel(cs, files, build) {
    if(cs == 0) {
      return
    }
    ...
    if(addCodeStyleExit == 0) {
      this.script.println "----------------------------------------------------------------------------"
      this.script.println "Gerrit Review: Code-Style=" + cs + " to change " + Change.changeJson._number + "/" +
        Change.sha1
      this.script.println "----------------------------------------------------------------------------"
    }
  ...
  private int generateAndPostLabelPayload(label, score, msgBody = "") {
    ...
    return postToGerrit("a/changes/" + Change.changeJson._number + "/revisions/" +
      Change.sha1 + "/review", jsonPayload)
  }
}
...
def prepareBuildsForEachMode(mode,tools,retryTimes,codestyle) {
    def builds = []
    if(codestyle) {
      builds += {
...
def buildChange() {
  ...
  modes.collect {
    prepareBuildsForEachMode(it,tools,1,Config.codeStyleBranches.contains(Change.branch))
  }.each { builds += it }
  ...
  if(codestyleResult) {
    ...
    gerrit.applyCodestyleLabel(resCodeStyle, findCodestyleFilesInLog(codestyleBuild), codestyleBuild)
  }
  ...
  gerrit.applyVerificationLabel(resVerify, Config.buildsList.findAll { key,build -> key != "codestyle" })
