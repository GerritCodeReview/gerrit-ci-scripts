...
def gerritLabelVerify(change, sha1, verified, builds) {
    ...
    def msgList = builds.collect { type,build ->
        [ 'type': type, 'res': build.getResult().toString(), 'url': build.getBuildUrl() +
            "consoleText" ]
...
def gerritLabel(changeNum, sha1, label, score, msgBody = "") {
    ...
    return gerritPost("a/changes/" + changeNum + "/revisions/" + sha1 + "/review", jsonPayload)
}
...
def buildChange(change) {
    ...
    modes.collect {
        buildsForMode(refspec,sha1,changeUrl,it,tools,branch,1,
            Globals.codeStyleBranches.contains(branch))
    }.each { builds += it }
    ...
    if(codestyleResult) {
        ...
        gerritLabelCodestyle(change, sha1, resCodeStyle, findCodestyleFilesInLog(codestyleBuild),
            codestyleBuild)
    }
    ...
    def resVerify = buildsWithResults.findAll{ it != codestyleResult }.
        inject(1) { acc, buildResult -> getVerified(acc, buildResult[1]) }

    def resAll = codestyleResult ? getVerified(resVerify, codestyleResult[1]) : resVerify

    gerritLabelVerify(change, sha1, resVerify,
...
queryUrl = new URL("${Globals.gerrit}changes/$requestedChangeId/?pp=0&O=3")
...
if(sha1 == null) {
    println "[WARNING] Skipping change " + changeJson.change_id +
