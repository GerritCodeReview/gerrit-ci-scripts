...
class Gerrit {
    ...
    int applyVerificationLabel(verified, builds) {
        if(verified == 0) {
            return;
        }
        ...
        def addVerifiedExit = generateAndPostLabelPayload('Verified', verified, msgBody)
        if(addVerifiedExit == 0) {
            this.script.println "----------------------------------------------------------------------------"
            this.script.println "Gerrit Review: Verified=${verified} to change " +
    ...
    static int applyCodestyleLabel(cs, files, build) {
        if(cs == 0) {
            return
        }
        ...
        def addCodeStyleExit = generateAndPostLabelPayload('Code-Style', cs, msgBody)
        if(addCodeStyleExit == 0) {
            this.script.println "----------------------------------------------------------------------------"
            this.script.println "Gerrit Review: Code-Style=${cs} to change " +
                "${Change.changeJson._number}/${Change.sha1}"
            this.script.println "----------------------------------------------------------------------------"
        }
    ...
    private int postToGerrit(url, jsonPayload) {
        ...
        def curl = ['curl',
            ...
            gerritPostUrl ]
        if(verbose) { this.script.println "CURL/EXEC> ${curl}" }
    ...
    private int generateAndPostLabelPayload(label, score, msgBody = "") {
        ...
        def jsonPayload = '{"labels":{"' + label + '":' + score + '},' +
            ' "message": "' + msgBody + '"' + notify + ", ${Config.addVerifiedTag} }"
...
def formatBuildStep(buildName,mode,retryTimes = 1) {
    def key = ""
...
def buildChange() {
    ...
    if (Config.codeStyleBranches.contains(Change.branch)){
        builds += formatBuildStep("Gerrit-codestyle", "reviewdb")
    }
    ...
    if(codestyleResult) {
        ...
        gerrit.applyCodestyleLabel(resCodeStyle, findCodestyleFilesInLog(codestyleBuild),
            codestyleBuild)
    }
    ...
    gerrit.applyVerificationLabel(resVerify, Builds.buildsList.findAll { key,build -> key != "codestyle" })
...
def retryFlakyBuilds(flaky){
    ...
    toolsAndModes.each {
        ...
        retryBuilds += formatBuildStep(buildName,mode,3)
    }
