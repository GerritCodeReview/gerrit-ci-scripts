...
case $TARGET_BRANCH$MODE in
  masterrbe|stable-3.11rbe|stable-3.12rbe|stable-3.13rbe|stable-3.14rbe)
    ...
    ;;
  masternotedb|stable-3.11notedb|stable-3.12notedb|stable-3.13notedb|stable-3.14notedb)
...
then
  ...
  then
    echo "Running screenshot tests in $(google-chrome --version)"
    bazelisk test $BAZEL_OPTS \
      //polygerrit-ui:web_test_runner_screenshots \
