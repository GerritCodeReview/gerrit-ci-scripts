...
case $TARGET_BRANCH$MODE in
  masterrbe|stable-3.6rbe|stable-3.7rbe|stable-3.8rbe|stable-3.9rbe)
    ...
    BAZEL_OPTS="$BAZEL_OPTS --config=remote_bb --remote_header=x-buildbuddy-api-key=$BB_API_KEY --remote_instance_name=projects/gerritcodereview-ci/instances/default_instance"
    ;;
