...
queryUrl = new URL(Globals.gerrit +
    "changes/?pp=0&o=CURRENT_REVISION&o=DETAILED_ACCOUNTS&o=DETAILED_LABELS&n=" +
    Globals.maxChanges + "&q=" + gerritQuery.encodeURL())
    ...
    change ->
        ...
        if(canBeVerified != null) {
            ...
            def myVerifications = verified.findAll {
                verification ->
                    verification._account_id == Globals.myAccountId && verification.value != 0
            }
...
if(buildsBandwith > 0) {
    ...
    def builds = changesTodo.collect { change ->
        { -> build(Globals.verifierJobName, CHANGE_ID: change) } }
    parallel(builds)
}
