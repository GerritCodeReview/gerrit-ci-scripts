...
then
  ...
  echo ""
  echo "Where: branch  Gerrit branch name where the release must but cut"
...
echo "Cloning and build Gerrit Code Review on branch $branch ..."
...
then
  echo "Version build is $warVersion while was expecting $version"
  exit 2
fi
